...
class CreateCanvTable(tk.Frame):
    def __init__(self,parent):
        ...
        self.canvas=tk.Canvas(self,yscrollcommand=self.vsb.set,bd=0,
...
class MyTitleTabl(tk.Frame):
    def __init__(self,parent,appdata):
        ...
        for itt in range(1,self.init.NUMFIELDS):
            self.titletabl.append(self.makelabel(self,h=30,
                                w=self.init.wid[itt],intext=self.init.name[itt]))
            self.titletabl[itt].grid(row=0,column=itt,
                                sticky='n'+'s'+'e'+'w',padx=1,pady=1)
            self.columnconfigure(itt,weight=1)
            
        self.titletabl.append(tk.Frame(self,width=2))
        self.titletabl[-1].grid(row=0,column=self.init.NUMFIELDS+1,sticky='e'+'w'+'n'+'s')
...
class Makeflag(tk.Frame):
    def __init__(self,parent,w):
        ...
        self.pack_propagate(0)
        self.flag=tk.IntVar()
        self.chbut=tk.Checkbutton(self,bg='#fff',variable=self.flag,state=tk.DISABLED)
...
class FastPan(tk.Frame):
    def __init__(self,parent):
        wbut=12; self.parent=parent
        tk.Frame.__init__(self,parent,width=100,height=20)
        cont=tk.Frame(self)
        self.butcopykz=tk.Button(cont,text='Скопировать\nкр. задерж.',width=wbut,
                                                      activebackground=abg)
        self.butcopyfr=tk.Button(cont,text='Скопировать\nфункц. распред.',width=wbut,activebackground=abg)
        self.butcopytabl=tk.Button(cont,text='Скопировать\nтаблицу',width=wbut,activebackground=abg) 
        self.butcopydat=tk.Button(cont,text='Скопировать\nаппрокс.',width=wbut,height=2,
                                                         activebackground=abg)
...
class MyPan(tk.Frame):
    def __init__(self,parent):
        wbut=10;self.parent=parent
        tk.Frame.__init__(self,parent,width=85,height=20)
        cont=tk.Frame(self)
        self.butadd=tk.Button(cont,text='Добавить',width=wbut,
                                                      activebackground=abg)
        self.butdel=tk.Button(cont,text='Удалить',width=wbut,activebackground=abg)
        self.butdelall=tk.Button(cont,text='Очистить',width=wbut,activebackground=abg) 
        self.butsavearr=tk.Button(cont,text='Сохранить \nв архив...',width=wbut,height=2,
                                                         activebackground=abg)
        self.butsavetxt=tk.Button(cont,text='Сохранить в \nтекст. файл...', width=wbut,height=2,
                                                          activebackground=abg)
        self.butload=tk.Button(cont,text='Загрузить...',width=wbut,activebackground=abg)
...
class CheckLabelBut(tk.Frame):
    def __init__(self,parent,text,cmd=None):
        ...
        self.chb=tk.Checkbutton(self.chframe,variable=self.flag,
                                command=cmd)
...
class SaveTxtDialog(tk.Toplevel):
    def __init__(self,parent):
        ...
        self.inpnt=EntrySaveTxt(panwsave,'Число точек аппроксимации: ',[2,1000,300])
        ...
        self.inpnt.grid(row=1,column=1,columnspan=2,padx=10,pady=15,sticky='s'+'w')
...
class AprxPanS(tk.Frame):
    ...
    def getdata(self):
        try:
            return {'pspl':int(self.snspl.getdata()),'npintr':int(self.snpint.getdata())}
        except:
            return None
...
class GeneralAprxPan(tk.Frame): 
    def __init__(self,parent):
        ...
        self.cmbselectaprx=ttk.Combobox(frSel,width=10,state='readonly',values=list(self.dict.keys()))
        ...
        tk.Label(frSel,text='СКО аппроксимации КЗ:').grid(column=0,row=0,sticky='w'+'e')
        self.entrsco.grid(column=0,row=1,sticky='w'+'e')
        tk.Label(frSel,text='Тип аппроксимации:').grid(column=0,row=2,sticky='w'+'e')
...
class IntegrScale(tk.Frame):
    def __init__(self,parent,sctype,intext):
        ...
        self.scale=tk.Scale(self,orient=tk.HORIZONTAL,length=200,sliderlength=20,
                       showvalue=0,resolution=0.5,command=self.onscale,bg=BG)
        ...
        self.inlim=0; self.max=0
        self.outlim=0; self.min=0
...
class DelPointDial(tk.Toplevel):
    def __init__(self,parent):
        ...
        self.butok = tk.Button(self, text='Ok',width=wid,state='disabled',command=self.okcmd,activebackground=abg)
        self.butcancel = tk.Button(self, text='Отмена',width=wid,command=self.onclosing,activebackground=abg)
...
class PlotWindow(tk.Toplevel):
    def __init__(self,parent):
        ...
        self.columnconfigure(0,weight=1)
        self.rowconfigure(0,weight=1); self.rowconfigure(1,weight=1)
    ...
    #Меню
        ...
        self.logshow=[tk.BooleanVar(),tk.BooleanVar(),tk.BooleanVar(),tk.BooleanVar()]
        self.showmenu.add_checkbutton(label='Аппроксимация', command=self.privatepltdata,
                                       variable=self.logshow[0])
        self.showmenu.add_checkbutton(label='Данные', command=self.privatepltdata,
                                       variable=self.logshow[1])
        self.showmenu.add_checkbutton(label='Ошибка', command=self.privatepltdata,
                                       variable=self.logshow[3])
        self.showmenu.add_checkbutton(label='Границы полиномов', command=self.privatepltdata,
                                       variable=self.logshow[2])
        self.logshow[2].set(0); self.logshow[1].set(1); self.logshow[0].set(1)
    ...
    def settrace(self,point=None):
        if point!=None:
            ...
            if point[1]>1:
                ...
                numF=point[1] 
            trace='Энергия {0} эВ; ФР {1} отн. ед.'.format('%.3f' %numW,'%.3f' %numF)
    ...
    def setmouseselectcmd(self,incmd):
        if incmd==0:
            ...
            self.button_release_event_id=self.canvi.mpl_connect('button_release_event',incmd)
    ...
    def pltdata(self,mdata=None,point=None,trace=None):
        if mdata==None:
            ...
            self.mouse_move_event_id=self.canvi.mpl_connect('motion_notify_event',self.tracecmd)
            self.mouse_move_event_id=self.canvdi.mpl_connect('motion_notify_event',self.tracecmd)
    ...
    def ploti(self,mdata,point=None,trace=None):
        ...
        if self.logshow[3].get()==1:
            ...
            plt.errorbar(mdata.data['w'],iplt, scoplt,fmt="o", color='m', ms=0,capsize=4)
        if point!=None:
...
class AppMenu(tk.Menu):
    ...
    def setfilecmd(self,incmd):
        ...
        self.filemenu.add_command(label='Открыть архив...', command=incmd['load'].execute)
        self.filemenu.add_command(label='Очистить', command=incmd['clear'].execute)
        self.filemenu.add_separator()
        self.filemenu.add_command(label='Сохранить в массив...', command=incmd['savearr'].execute)
        self.filemenu.add_command(label='Сохранить в текстовый файл...', command=incmd['savetxt'].execute)
        self.filemenu.add_separator()
        self.filemenu.add_command(label='Выход', command=incmd['quit'])
        self.setmenu.add_command(label='Установить всем...',command=incmd['setall'].execute)
        self.setstate(0)
        self.cansave(0)
