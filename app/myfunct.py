...
def intSimscum(lim, nInt, fName, nOut=100):
    ...
    for i in range(1, nump - 1, 2):
        intC[int((i + 1) / 2)] = intC[int((i + 1) / 2) - 1] + h / 3 * (
            fName(ld + i * h - h) + 4 * fName(ld + i * h) + fName(ld + i * h + h)
        )
    return intC
    ...
    def __init__(self, xi, yi, y2sh=[0, 0]):
        ...
        try:
            ...
            for itt in range(self.n - 1):
                hi1 = xi[itt + 1] - xi[itt]
                self.spline[itt]["d"] = (
                    (self.spline[itt + 1]["c"] - self.spline[itt]["c"]) / hi1 / 3
                )
                self.spline[itt]["b"] = (yi[itt + 1] - yi[itt]) / hi1 - hi1 / 3 * (
                    2 * self.spline[itt]["c"] + self.spline[itt + 1]["c"]
                )
...
class B_Spline:
    def __init__(self, xi, yi, w=None, npintr=2, pspl=2):
        ...
        try:
            ...
            for itt in range(1, self.nspl):
                self.x.append((xi[dn + itt * npintr] + xi[dn + itt * npintr - 1]) / 2.0)
    ...
    def scalmultB(self, ispl, jspl):
        ...
        for itt in range(self.ndat):
            out += (
                self.calcB(ispl, self.xi[itt])
                * self.calcB(jspl, self.xi[itt])
                * self.w[itt] ** 2
            )
        return out
    ...
    def calcB(self, ispl, x):
        ...
        for pi in range(p):
            for itt in range(p - pi):
                ...
                B[itt] = (x - xn) * B[itt] / (xnp1 - xn) + (xnp2 - x) * B[itt + 1] / (
                    xnp2 - xn1
                )
        return B[0]
    ...
    def calcdB(self, ispl, x):
        ...
        for pi in range(p):
            for itt in range(p - pi):
                ...
                dB[itt] = ((x - xn) * dB[itt] + B[itt]) / (xnp1 - xn) + (
                    (xnp2 - x) * dB[itt + 1] - B[itt + 1]
                ) / (xnp2 - xn1)
                B[itt] = (x - xn) * B[itt] / (xnp1 - xn) + (xnp2 - x) * B[itt + 1] / (
                    xnp2 - xn1
                )
