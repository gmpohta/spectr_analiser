...
class SaveTxtCommand(Command):
    def __init__(self, appdata, arrdata, pltwind):
        ...
        cmd = {
            "save": self.cmdsave,
            "sel": self.selectall,
            "unsel": self.unselectall,
            "norm": self.normalmode,
        }
        self.top.setcmd(cmd)
    ...
    def cmdsave(self, npcalc, wsave):
        ...
        if alls != []:
            pfile = fdial.asksaveasfilename(
                defaultextension=".txt",
                initialfile="noname.txt",
                title="Сохранить в текстовый файл",
            )
            if pfile:
                f = open(pfile, "w")
...
class SaveArrCommand(Command):
    ...
    def execute(self, ev=None):
        pfile = fdial.asksaveasfilename(
            defaultextension=".pkl", initialfile="newarr.pkl", title="Сохранить в архив"
        )
        if pfile:
            try:
                f = open(pfile, "wb")
                ...
                msgbox.showerror(
                    "Open Source File", "Failed to save file\n'%s'" % pfile
                )
            except:
...
class LoadArrCommand(Command):
    ...
    def execute(self):
        pfile = fdial.askopenfilename(
            filetypes=(("Пакет данных", "*.pkl"), ("All files", "*.*"))
        )
        if pfile:
            try:
                f = open(pfile, "rb")
                ...
                msgbox.showerror(
                    "Open Source File", "Failed to load file\n'%s'" % pfile
                )
            except:
                msgbox.showerror("Open Source File", "Wrong type file\n'%s'" % pfile)
                f.close()
        ...
        if msgbox.askyesno("Очистить?", "Вы уверены?"):
            ...
            if num != 0:
                for itt in range(num - 1, -1, -1):  # Переменная itt проходит все строки
                    self.arrdata[
                        itt
                    ].destroyGUI()  # с конца в начало, чтобы последующие строки не смещались
    ...
    def execute(self):
        ...
        for itt in self.arrdata:
            temp += (
                itt.rowdata["kname"]
                + "\t"
                + str(round(itt.rowdata["n"], 0))
                + "\t"
                + str(round(itt.rowdata["wavr"], 3))
                + "\t"
                + str(round(itt.rowdata["wmax"], 3))
                + "\n"
            )
...
class TableLine:
    ...
    def repack(self, numrow):
        self.table[0].grid(
            row=numrow, column=0, sticky="w" + "e" + "n" + "s", padx=1, pady=1
        )
        for itt in range(1, len(self.table)):
            self.table[itt].grid(
                row=numrow, column=itt, sticky="w" + "e" + "n" + "s", padx=1, pady=1
            )
    ...
    def accept(self):
        self.updaterowdata()
        if (
            self.rowdata["mgas"] != self.NODATAROW
            and self.rowdata["scol"] != self.NODATAROW
        ):
            return True
        else:
            return False
    ...
    def calcDens(self):
        if self.accept():
            ...
            if self.rowdata["n"] != self.data.NODATAROW:
                ...
                self.table[4].refresh(self.NODATAROW)
            if (
                self.rowdata["wavr"] != self.NODATAROW
                and self.rowdata["wmax"] != self.NODATAROW
            ):
                self.table[5].refresh(round(self.rowdata["wavr"], 2))
                self.table[6].refresh(round(self.rowdata["wmax"], 2))
            else:
...
class InitData:
    ...
    def __init__(self):
        self.NUMFIELDS = (
            7  # #Количество колонок таблицы(включая колонку флага выделения строки)
        )
        self.name = [
            "",
            "Кадр",
        ...
        self.NOROWDATA = {
            "kname": InitData.NODATAROW,
            "scol": InitData.NODATAROW,
            "mgas": InitData.NODATAROW,
            "n": InitData.NODATAROW,
            "wavr": InitData.NODATAROW,
            "wmax": InitData.NODATAROW,
        }
...
class MainData(InitData):
    def __init__(self, data, kname):
        ...
        self.datares = {
            "w": [["w_э, эВ"] + data[0].copy()],
            "i": [["icol_э, мкА"] + data[1].copy()],
            "sco": [["sco, мкА"] + data[2].copy()],
        }
        ...
        self.data = {
            ...
            "i": data[1],
            "sco": data[2],
        }  # Данные + аппроксимированные данные+ ошибки данных
        self.limits = {
            "max": self.data["w"][-1],
            "min": self.data["w"][0],
            "lup": self.data["w"][-1],
            "ldw": self.data["w"][0],
        }  # Пределы интегрирования (и сохр. данных)
    ...
    def detpoint(self, point):
        ...
        wm = point[2]
        rmin = (
            (w[0] / wm - point[0] / wm) ** 2 + (i[0] / im - point[1] / im) ** 2
        ) ** 0.5
        ind = 0
        for itt in range(1, len(w)):
            rcurr = (
                (w[itt] / wm - point[0] / wm) ** 2 + (i[itt] / im - point[1] / im) ** 2
            ) ** 0.5
            if rmin >= rcurr:
                rmin = rcurr
                ind = itt
...
class AppoxProc:
    ...
    def calcxP(self, mdata, nump, inputvar=None):
        ...
        if inputvar != None:
            return [
                ["w_a, эВ"] + linew.tolist(),
                ["icol_a, мкА"] + linei.tolist(),
                ["f(w)_a, отн.ед"] + linedi,
            ]
        else:
    ...
    def integrP(self, limits):
        ...
        self.rowdata["n"] = myf.intSims(
            [limits["ldw"], limits["lup"]], self.NUMPINT, self.functFRP
        )
        if self.rowdata["n"] == 0:
            wavr = limits["ldw"]
        else:
            wavr = (
                myf.intSims([limits["ldw"], limits["lup"]], self.NUMPINT, functwavr)
                / self.rowdata["n"]
            )
    ...
    def doaprxS(self, mdata, weigt=None):
        ...
        yi = mdata.data["i"]
        scoi = mdata.data["sco"]
        self.aprxS = myf.B_Spline(
            xi,
            yi,
            npintr=self.aprxdataS["npintr"],
            pspl=self.aprxdataS["pspl"],
            w=weigt,
        )
    ...
    def integrS(self, limits):
        ...
        self.rowdata["n"] = myf.intSims(
            [limits["ldw"], limits["lup"]], self.NUMPINT, self.functFRS
        )
        if self.rowdata["n"] > 1:
            n0 = self.rowdata["n"]
        wavr = myf.intSims([limits["ldw"], limits["lup"]], self.NUMPINT, functavrg) / n0
        if np.isnan(wavr):
            wavr = limits["ldw"]
        self.rowdata["wavr"] = wavr
...
class MyApp(tk.Tk):
    def __init__(self):
        ...
        self.canvtabl.grid(
            row=0, column=1, columnspan=3, sticky="w" + "e" + "s" + "n", pady=5
        )
        self.pan.grid(row=0, column=0, sticky="s" + "n")
        self.genaprxpan.grid(
            row=1, column=1, columnspan=4, sticky="w" + "e" + "s" + "n", pady=0, padx=10
        )
